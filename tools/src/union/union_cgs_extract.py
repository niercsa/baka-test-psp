...
import gzip
def cgs_extract():
    os.makedirs(os.path.dirname("1_extracted/cgs/"), exist_ok=True)
    cwd = os.getcwd()
    meta = json.loads(open("tools/src/union/cgs.json", "rb").read())
    print(cwd)
    for union_index in meta:
        f = open("1_extracted/union/0{}.bin".format(union_index), "rb")
        info = meta[str(union_index)]
        open("1_extracted/cgs/0{}.json".format(union_index), 'wb').write(json.dumps(info, indent=4).encode("utf-8"))
        pixel_index = 1
        for pal, pix in zip(info["content"]["pallete"], info["content"]["pixel"]):
            ...
            for off, size in zip(pix["chunk_offset"], pix["gzip_csizes"]):
                ...
                pixel += gzip.decompress(f.read(size-16))
            gim = open("1_extracted/temp.gim", 'wb')
            ...
            gim.close()
            os.system(
                "tools\\GimConv\\GimConv.exe 1_extracted/temp.gim -o {2}/1_extracted//cgs/{0}_{1:02d}.png".format(
                    union_index, pixel_index, cwd))
            os.remove("1_extracted/temp.gim")
            pixel_index += 1
        #if info["content"]["map"]:
